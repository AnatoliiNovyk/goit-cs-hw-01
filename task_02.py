...
# Крок 1: Розширення лексера - додано нові токени
class TokenType:
    INTEGER = "INTEGER"
    PLUS = "PLUS"
    MINUS = "MINUS"
...
class Parser:
    ...
    def parse(self):
        """Запускає парсинг та перевіряє, чи не залишилось зайвих токенів."""
        node = self.expr()
        if self.current_token.type != TokenType.EOF:
            self.error()
        return node

# Крок 3: Оновлення інтерпретатора
class Interpreter:
